...
ETHEREUM_DIR = coin_info.DEFS_DIR / "ethereum"

TIMESTAMP_PATH = ETHEREUM_DIR / "latest_definitions_timestamp.txt"
DEFINITIONS_PATH = ETHEREUM_DIR / "definitions.json"
CACHE_PATH = ETHEREUM_DIR / "definitions-cache.json"
NETWORKS_PATH = ETHEREUM_DIR / "chains"
TOKENS_PATH = ETHEREUM_DIR / "tokens"
...
def get_testnet_status(*strings: str) -> bool:
    for s in strings:
...
def _load_ethereum_networks_from_repo() -> coin_info.Coins:
    """Load ethereum networks from submodule."""
    networks = []
    for chain in NETWORKS_PATH.glob("eip155-*.json"):
        chain_data = coin_info.load_json(chain)
...
def _create_cropped_token_dict(
    complex_token: coin_info.Coin, chain_id: int, chain: str
) -> dict | None:
    # simple validation
...
def _load_erc20_tokens_from_coingecko(
    downloader: Downloader, networks: list[dict]
) -> list[dict]:
    tokens: list[dict] = []
...
def _load_erc20_tokens_from_repo(networks: coin_info.Coins) -> coin_info.Coins:
    """Load ERC20 tokens from submodule."""
    tokens = []
    for network in networks:
        chain_path = TOKENS_PATH / network["chain"]
        for file in chain_path.glob("*.json"):
            token = coin_info.load_json(file)
...
def _load_prepared_definitions(
    ...
    prepared_definitions_data = coin_info.load_json(definitions_file)
    try:
    ...
    networks: coin_info.Coins = []
    for network_data in networks_data:
        network_data.update(
            chain_id=network_data["chain_id"],
            key=f"eth:{network_data['shortcut']}",
        )
        networks.append(cast(coin_info.Coin, network_data))

    tokens: coin_info.Coins = []
    ...
    for token in tokens_data:
        ...
        tokens.append(cast(coin_info.Coin, token))
...
def load_raw_builtin_ethereum_networks() -> list[dict]:
    ...
    return coin_info.load_json("ethereum", "networks.json")


def load_raw_builtin_erc20_tokens() -> list[dict]:
    """Load ERC20 tokens from `ethereum/tokens.json`."""
    tokens_data = coin_info.load_json("ethereum", "tokens.json")
...
def eth_info_from_dict(
    coin: coin_info.Coin, msg_type: EthereumNetworkInfo | EthereumTokenInfo
...
@click.option(
    "-o",
    "--outfile",
    type=click.Path(resolve_path=True, dir_okay=False, writable=True, path_type=Path),
...
def sign_definitions(
    ...
    def save_definition(
        path: PurePath, keys: list[str], data: bytes, zip_file: zipfile.ZipFile
    ):
    ...
    def generate_token_def(token: Coin, base_path: PurePath, zip_file: zipfile.ZipFile):
    ...
    def generate_network_def(
        network: Coin, base_path: PurePath, zip_file: zipfile.ZipFile
    ):
    ...
    base_path = PurePath("definitions-latest")
