...
# ---------------------------------------------------------------------
# 4c. Calldata-bound fields (per-formatter `params` handling: params.py)
# ---------------------------------------------------------------------
...
def _build_path_field(
    ...
    # Deferred: `.params` imports back from this module (`_FormatContext`,
    # `_FORMATTER_MAP`, ...), so importing it at module level would be
    # circular. By call time both modules are fully loaded.
    from .params import (
        apply_calldata_params,
        apply_date_params,
        apply_enum_params,
        apply_token_amount_params,
        apply_unit_params,
    )
    ...
    if fmt == "tokenAmount":
        ...
        apply_date_params(out, params, path_str, label, ctx)
    elif fmt == "calldata":
        apply_calldata_params(out, params, path_str, label, ctx)
    elif fmt == "enum":
        apply_enum_params(out, params, path_str, label, ctx)
    return out
